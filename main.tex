...
A single electron (or filament beam) traveling in an undulator with pure sinusoidal magnetic field in one direction will produce a complicated wavefront with geometry that varies as a function of the photon frequency (see, e.g., Ref.~\cite{elleaume}). At the resonance energy the angular emission in the far field can be approximated by a Gaussian of width \cite{elleaume}
\begin{equation}
...
The Fresnel integral propagator can be seen as convolution of the wavefield with a Gaussian kernel. One can write Equation \ref{eq:fresnelPropagator} in convolution form, involving two-Fourier transforms:
...
In order to calculate the absorbed power density at the mirror, it is necesary to apply the energy-dependent mirror reflectance on the power density for different energies. There are several codes that  can be computed this, like SPECTRA \cite{codeSPECTRA}, SRCALC-IDPOWER \cite{SRCALC} and XOPPY. The last two codes are available in the OASYS environment. Table~\ref{tab:powerValues} shows the results in terms of power absorbed and peak power density as calculated by these three codes. 

\begin{table}\label{tab:powerValues}
    \caption{Absorbed power by the mirror M1 in different apertures as calculated by different codes. \todo{remove incomplete tables V and Circ}}
    \resizebox{\textwidth}{!}{
    \begin{tabular}{lccc}
H polarization & SPECTRA & SRCALC & XOPPY \\
Power (W) in 500$\times$10 mm$^2$ & 2070 & 2489 & 2347     \\
Power (W) in 100$\times$2.4 mm$^2$ & 239 & 306 & 245     \\
Peak power density (W/mm$^2$) & 1.01 & 1.26 & 1.18     \\
\hline\hline
V polarization & SPECTRA & SRCALC & XOPPY \\
Power (W) in 500$\times$10 mm$^2$ & \inred{Grant?} & 2319 & -     \\
Power (W) in 100$\times$2.4 mm$^2$ & \inred{Grant?} & 292 & -     \\
Peak power density (W/mm$^2$) & \inred{Grant?} & 1.24 & -     \\
\hline\hline
Circ polarization & SPECTRA & SRCALC & XOPPY \\
Power (W) in 500$\times$10 mm$^2$ & - & 535 & -     \\
Power (W) in 100$\times$2.4 mm$^2$ & - & 3.14 & -     \\
Peak power density (W/mm$^2$) & - & 0.42 & -     \\
    \end{tabular}
    }
\end{table}
