...
Using data from a 18-channel adaptive X-ray mirror prototype, we implemented mirror shape-control algorithms designed to restore and optimize the focused beam intensity (i.e., Strehl ratio).
...
\begin{table}\label{tab:powerValues}
    \caption{Absorbed power by the mirror M1 for two apertures (open slits 500 $\times$ 10 mm$^2$ and 6$\sigma$ 100 $\times$ 24 mm$&^2$) as calculated by different codes. \todo{remove incomplete tables V and Circ}}
    \resizebox{\textwidth}{!}{
    \begin{tabular}{lccc}
H polarization & SPECTRA & SRCALC & XOPPY \\
Power (W) in 500$\times$10 mm$^2$ & 2070 & 2489 (67.1\%) & 2347 (65.6\%)    \\
Power (W) in 100$\times$2.4 mm$^2$ & 239 & 306 (8.3\%) & 245  (6.9\%)   \\
Peak power density (W/mm$^2$) & 1.01 & 1.26 (80.3\%) & 1.18 (74.5\%)    \\
\hline\hline
V polarization & SPECTRA & SRCALC & XOPPY \\
Power (W) in 500$\times$10 mm$^2$ & \inred{Grant?} & 2319 (66.7\%) & -     \\
Power (W) in 100$\times$2.4 mm$^2$ & \inred{Grant?} & 292 (8.4\%) & -     \\
Peak power density (W/mm$^2$) & \inred{Grant?} & 1.24 (79.3\%) & -     \\
\hline\hline
Circ polarization & SPECTRA & SRCALC & XOPPY \\
Power (W) in 500$\times$10 mm$^2$ & - & 535 (42.6\%) & -     \\
Power (W) in 100$\times$2.4 mm$^2$ & - & 3.14 (0.3\%) & -     \\
Peak power density (W/mm$^2$) & - & 0.42 %(69.7\%)
& -     \\
    \end{tabular}
    }
\end{table}

The distortion of the mirror was calculated using the finite-element method (FEM), as implemented in ANSYS software for the horizontal and vertical polarizations. The case of circular polarizations has not been considered because of the small absorbed power. The FEM calculation generates a map of the surface deformation for a given heat load and mirror design, including cooling. Figure~\ref{fig:M1deformation} shows the deformation maps (column 1) and the tangential profile at the mirror center (column 2). The maps were loaded in OASYS using a dedicated widget that also extracts the 1D profile and sends it to the reflector.
...
iii) it calculates the phase difference $\Delta \phi(x) = \phi_{sph} - \phi_{inc}$, and iv) it computes the profile height $h(w) = -\Delta \phi(w) / (2 k \sin \theta)$. \todo{MENTION THE AMPLITUDE WEIGHTING} In this model, the wavefront sensor has an ideal response.
