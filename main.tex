...
The distortion of the mirror was calculated using the finite-element method, as implemented in ANSYS software. The calculation generates a map of the surface deformation for a given heat load and mirror design, including cooling. Figure~\ref{fig:M1deformation} shows the deformation maps (column 1) and the extracted profile (column 2). The maps were loaded in OASYS using a dedicated widget that also extracts the 1D profile and sends it to the reflector.
