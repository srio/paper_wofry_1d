...
% For discrete wavefronts this integral reduces to
...
%The Fresnel propagator \cite{paganin} gives a recipe to compute the transported wavefield at a position $y_1$ as a function of the wavefield at position $y_0$
%
%\begin{equation}\label{eq:fresnelPropagator}
%E(x;y_1) =  \frac{e^{i k (y_1-y_0)}}{\sqrt{i \lambda (y_1-y_0)}} \int E(x';y_0) e^{ \frac{i k}{2 (y_1-y_0)}  (x-x')^2  }  dx'.
%\end{equation}
...
%code with the absorption curve calculated for the mirror's grazing angle and coating .  The peak absorbed power density on the mirror is 1.01 W/mm$^2$. The total absorbed power in a $\pm 3 \sigma$ aperture is approximately 239 W, where $\sigma$ is the width of the spatial distribution of the first harmonic at $K=3.07$, corresponding to an energy of 231 eV.
...
%it manages to cancel the deformation with resulting Strehl ratio close to one (again, it could be higher than one when the intensity sreads over M3 incrementing the NA). 
